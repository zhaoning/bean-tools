...
def gen_journals(request):
    """Process directive by calling appropriate handlers.
    """
    req = request.copy()
...
def requests_from_string(text):
    """Extract a list of requests from given string.
    """
...
def requests_from_file(filename):
    """Extract a list of requests from given filename.
    """
    with open(filename, 'r') as f:
        text = f.read()
    return requests_from_string(text)
...
if not hasattr(sys, 'ps1'):
    ...
    if args.files:
        requests = itertools.chain.from_iterable(
                [requests_from_file(f) for f in args.files]
                )
    else:
        requests = requests_from_string(sys.stdin.read())

    journals = itertools.chain.from_iterable(
            [gen_journals(r) for r in requests]
            )
